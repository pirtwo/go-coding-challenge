...
01 70 54 71 83 51 54 69 16 92 33 48 61 43 52 01 89 19 67 48`
	...
	grid := createGrid(extractNumbers(str), 20, 20)
	max := findLargestProduct(grid, 4)
	fmt.Println(max)
}
...
func createGrid(numbers []int, rows int, cols int) [][]int {
	grid := make([][]int, rows)
	for i := 0; i < rows; i++ {
		grid[i] = make([]int, cols)
		for j := 0; j < cols; j++ {
			grid[i][j] = numbers[i*cols+j]
		}
	}
	return grid
}

func findLargestProduct(grid [][]int, adjLen int) Adjacent {
	rows := len(grid)
	cols := len(grid[0])
	currMax := Adjacent{0, nil}

	for i := 0; i < rows; i++ {
		for j := 0; j < cols; j++ {

			adjs := make([]Adjacent, adjLen)

			for l := 0; l < len(adjs); l++ {
				adjs[l].sum = 1
			}
			...
			for k := 0; k < adjLen; k++ {
				// right adj
				if j+adjLen-1 < cols {
					adjs[0].sum *= grid[i][j+k]
					adjs[0].members = append(adjs[0].members, grid[i][j+k])
				}

				// bottom adj
				if i+adjLen-1 < rows {
					adjs[1].sum *= grid[i+k][j]
